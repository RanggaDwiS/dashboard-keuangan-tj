...
if uploaded_file:
    ...
    try:
        ...
        with c_kanan:
            # 📈 COMBO CHART: MENGGUNAKAN SECONDARY Y-AXIS AGAR LINE MENIMPA BAR
            fig_combo = make_subplots(specs=[[{"secondary_y": True}]])

            # 1. Bar Chart (Sumbu Y Kiri)
            fig_combo.add_trace(
                go.Bar(
                    x=df_kat['Waktu'],
                    y=df_kat['Total Pendapatan'],
                    name='Total Pendapatan',
                    marker_color='#8ECAE6', # Biru Muda
                    text=df_kat['Total Pendapatan'],
            ...
            fig_combo.add_trace(
                go.Scatter(
                    ...
                    line=dict(color='#023047', width=4), # Biru Tua
                    marker=dict(size=10, line=dict(width=2, color='white')),
                    text=df_kat['Laba/Rugi'],
                    texttemplate='%{text:,.0f}',
            ...
            # Sembunyikan garis grid dan sumbu Y agar clean (Executive Style)
            ...
            # ⚖️ BALANCE SHEET (Liabilitas & Ekuitas)
            df_stack = pd.melt(df_kat, id_vars=['Waktu'], value_vars=['Total Liabilitas', 'Total Ekuitas'])
            fig_bar = px.bar(df_stack, x='Waktu', y='value', color='variable', 
                             color_discrete_map={'Total Liabilitas':'#E63946', 'Total Ekuitas':'#2A9D8F'},
                             text_auto=',.0f')
            fig_bar.update_traces(textposition='inside')
            fig_bar.update_layout(separators=",.", barmode='group', height=200, margin=dict(t=10,b=0,l=0,r=0), 
                                  showlegend=True, legend=dict(orientation="h", yanchor="bottom", y=1.1, xanchor="right", x=1, title=""), 
                                  yaxis_visible=False, xaxis_title=None)
